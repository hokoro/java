...
class Process {
	...
	/* 아래 각 함수를 구현할 것 */
	public void	   incExecTime() 			{ executionTime++; } /* 프로세스의 실행시간을 증가시킴 */
	public int     getWaitingTime(int cTime){ return cTime - arrivalTime; } /* 대기시간을 계산해 옴 */
	public int     getRemainingTime()		{ return serviceTime - executionTime; } /* 남을 실행시간을 계산해 옴 */
	public String  getName()  				{ return name; } /* 프로세스의 이름을 얻어옴 */
	public boolean isFinished()				{ if(serviceTime==executionTime) return true; else return false; } /* 프로세스의 실행이 종료되었는지 체크함 */
}
...
class RR extends Scheduler {
	...
	public void schedule()
	{
		...
		if(execTime>=quantum) {				//실행시간이 quantum 을 넘어가면 선점형 스케줄링 의 조건 
			if(readyQueue.peek()!=null)			//현재 프로세스의 데이터 있다면
				readyQueue.add(readyQueue.remove());		//현재 존재하는 데이터를 제거하고 바로 뒤에 추가해준다
		}
		super.schedule();			//현재 프로세스가 종료됬는지 확인
		if(readyQueue.peek()!=currentProcess)			//만약 프로세스가 바뀌는 경우는 quantum 을 다 소진했을떄
			execTime=0;								//execTime 이 0 으로 초기화
		currentProcess=readyQueue.peek();						//현재 프로세스에 다음 프로세스를 추가
	...
	public void clockInterrupt()
	{
		super.clockInterrupt();			//맨먼저 스케줄러 함수에 clocktime 을 불러줘야 함 -> 프로세스 출력 및 조건 확인
		execTime++;						//실행시간 증가
		if ( execTime>=quantum||isNewProcessArrived || ((currentProcess != null) && currentProcess.isFinished())) 
		 {		//1.만약 실행시간이 조건시간을 같거나 넘어갔을떄 2.새로운 프로세스가 도착했을때 3.프로세스가 실행되고 포로세스가 종료 됬을떄 
			schedule();		
		 }
	}
}
public class SchedulingApp_20164229 {
